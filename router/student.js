...
router.post("/", async (req, res) => {
    try {
        ...
        const newstudent = await new Student(student).save().then((res) => { return res }).catch((err) => { throw err.message })
...
router.post("/update/:id", async (req, res) => {
    const { id } = req.params
    try {
        const updateuser = await Student.updateMany({ "_id": id }, { $set: req.body }).then((res) => { return res }).catch((err) => { throw err.message })
